student_data <- read.csv("C:/Users/anika/Data Science Project/student_performance_data1.csv")
head(student_data)
str(student_data)
...
for(col in numeric_cols) {
  ...
  lower_bound <- Q1 - 1.5 * IQR_value
  upper_bound <- Q3 + 1.5 * IQR_value
  outliers <- student_data[!is.na(x) & (x < lower_bound | x > upper_bound), ]
  cat("\n============================\n")
...
correlation_value
student_data <- subset(student_data, select = -c(Height))
"Height" %in% colnames(student_data)

correlation_value <- cor(student_data$StudentID, student_data$GPA, use = "complete.obs")
correlation_value
student_clean_data <- subset(student_data, select = -c(StudentID))
"StudentID" %in% colnames(student_clean_data)
